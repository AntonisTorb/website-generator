    ...
    data = {
        ...
        "user_work_history": [
            ...
            },
            {
                ...
                "user_work_responsibilities": [
                    "Knowledge transfer in preparation for the national university entry exams."
                ],
            },
        ],
        "user_education": [
        ...
        "user_languages": [
            ...
            (
                "🇬🇧",
                "English",
                "IELTS General Training (01/2019), Overall Score: 8/9 (C1 level). High everyday usage, high confidence.",
            ),
            (
                "🇩🇪",
                "German",
                "Zertifikat B2 (05/2009), Overall Score: 80.5/100. Very low circumstantial usage, low confidence.",
            ),
        ],
        "user_accomplishments": [
            (
                "./assets/images/eeom.jpeg",
                "Received the Employee of the Month award for October 2019 during my employment with Eli Lilly.",
            ),
            (
                "./assets/images/army.jpg",
                "Received 14 additional days of honorary leave awarded for my contributions during my military service.",
            ),
        ],
        "user_projects": [
            {
...
def generate_html():
    ...
    for position in data["user_work_history"]:
        ...
        for user_responisibility in user_work_responsibilities_list:
            responsibility = user_work_responsibilities_tmp().format(
                user_responisibility=user_responisibility
            )
            user_work_responsibilities = (
                f"{user_work_responsibilities}\n{responsibility}"
                if user_work_responsibilities
                else responsibility
            )
    ...
    for lang_icon, lang_name, lang_desc in data["user_languages"]:
        language = user_languages_tmp().format(
            lang_icon=lang_icon, lang_name=lang_name, lang_desc=lang_desc
        )
    ...
    for accomplishment_link, accomplishment_desc in data["user_accomplishments"]:
        accomplishment = user_accomplishments_tmp().format(
            accomplishment_link=accomplishment_link,
            accomplishment_desc=accomplishment_desc,
        )
        user_accomplishments = (
            f"{user_accomplishments}\n{accomplishment}"
            if user_accomplishments
            else accomplishment
        )
    ...
    for user_project in data["user_projects"]:
        ...
        for project_description_paragraph in user_project_description_list:
            ...
            user_project_description = (
                f"{user_project_description}\n{description_paragraph}"
                if user_project_description
                else description_paragraph
            )
